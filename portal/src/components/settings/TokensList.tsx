...
export const TokenRow = ({ token }: TokenRowProps) => {
  ...
  const onDeleteClick = useCallback(async () => {
    try {
      await axios.delete(`/api/${portalName}/tokens`, {
        data: {
          name: token.name,
        },
      })
      queryClient.invalidateQueries({ queryKey: [`/api/${portalName}/tokens`] })
      addToast({
        severity: 'success',
  ...
  const onSuspendClick = useCallback(async () => {
    try {
      await axios.post(`/api/${portalName}/tokens/suspend`, {
        name: token.name,
...
export const TokensListWithData = () => {
  ...
  const fetchUrl = `/api/${portalName}/tokens`
